...
function prepare_am_config_file() {
  ...
  printf "%s | [DEBUG]        DS_DIRMGRPASSWD: \"%s\"\n" "$(date +"%Y-%m-%d %H:%M:%S")" "$AM_CONFIG_STORE_MANAGER_PASSWORD"
  ...
  printf "%s | [DEBUG]       USERSTORE_PASSWD: \"%s\"\n" "$(date +"%Y-%m-%d %H:%M:%S")" "$AM_USER_STORE_MANAGER_PASSWORD"
  ...
  sed -i "/^DS_DIRMGRPASSWD=/c\DS_DIRMGRPASSWD=$AM_CONFIG_STORE_MANAGER_PASSWORD" "$AM_CONFIG_FILE"
  ...
  sed -i "/^#USERSTORE_PASSWD=/c\USERSTORE_PASSWD=$AM_USER_STORE_MANAGER_PASSWORD" "$AM_CONFIG_FILE"
}
