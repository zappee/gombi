...
function install_ds() {
  ...
  local server_id profile_config profile_store
  server_id="master-ldap"
  profile_config="am-config"
  profile_store="am-identity-store"

  local base_dn
  base_dn=$(fqdn_to_ldap_dn "$fqdn")

  printf "%s | [INFO]  installing ForgeRock Directory Service...\n" "$(date +"%Y-%b-%d %H:%M:%S")"
  printf "%s | [DEBUG]                    DS_HOME: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$DS_HOME"
  printf "%s | [DEBUG]                  server-id: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$server_id"
  printf "%s | [DEBUG]                   profiles: \"%s\", \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$profile_config" "$profile_store"
  ...
  printf "%s | [DEBUG]          keystore password: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$keystore_password"
  printf "%s | [DEBUG]                    base dn: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$base_dn"
  ...
  ./setup \
    ...
    --acceptLicense \
    --set am-config/backendName:$profile_config \
    --set am-config/baseDn:ou=$profile_config,$base_dn \
    --set am-config/amConfigAdminPassword:"$LDAP_USER_PASSWORD" \
    --set am-identity-store/backendName:$profile_store \
    --set am-identity-store/baseDn:ou=$profile_store,$base_dn \
