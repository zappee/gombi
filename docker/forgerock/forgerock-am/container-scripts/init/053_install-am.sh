...
# ------------------------------------------------------------------------------
function install_am() {
  local fqdn keyStoreType keyStore keyStorePassword truststore trustStorePassword
  fqdn=$(hostname -f)
  keyStoreType="PKCS12"
  keyStore="/tmp/$fqdn.p12"
  keyStorePassword="changeit"
  truststore="$keyStore"
  trustStorePassword="$keyStorePassword"

  printf "%s | [INFO]  installing ForgeRock Access Management Server...\n" "$(date +"%Y-%b-%d %H:%M:%S")"
  printf "%s | [DEBUG]                AM_HOME: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$AM_HOME"
  printf "%s | [DEBUG]    AM_CONFIG_TOOL_HOME: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$AM_CONFIG_TOOL_HOME"
  printf "%s | [DEBUG]         AM_CONFIG_TOOL: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$AM_CONFIG_TOOL"
  printf "%s | [DEBUG]         AM_CONFIG_FILE: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$AM_CONFIG_FILE"
  printf "%s | [DEBUG]          keystore type: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$keyStoreType"
  printf "%s | [DEBUG]               keystore: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$keyStore"
  printf "%s | [DEBUG]      keystore password: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$keyStorePassword"
  printf "%s | [DEBUG]             truststore: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$truststore"
  printf "%s | [DEBUG]    truststore password: \"%s\"\n" "$(date +"%Y-%b-%d %H:%M:%S")" "$trustStorePassword"
  ...
  "$JAVA_HOME/bin/java" \
     ...
     -jar "$AM_CONFIG_TOOL_HOME/openam-configurator-tool-14.1.3.18.jar" \
     --file "$AM_CONFIG_FILE" \
     --acceptLicense
...
java -Djavax.net.debug=all -Djavax.net.ssl.keyStoreType=PKCS12 -Djavax.net.ssl.keyStore=/tmp/am.hello.com.p12 -Djavax.net.ssl.keyStorePassword=changeit -Djavax.net.ssl.trustStore=/tmp/am.hello.com.p12 -Djavax.net.ssl.trustStorePassword=changeit -jar openam-configurator-tool-14.1.3.18.jar --file config.properties
...
function prepare_am_config_file() {
  ...
  am_url="https://$fqdn:$EXPOSED_HTTPS_PORT"
  ...
  sed -i "/^ROOT_SUFFIX=/c\ROOT_SUFFIX=$AM_CONFIG_STORE_ROOT_SUFFIX" "$AM_CONFIG_FILE"
  ...
  sed -i "/^#USERSTORE_SUFFIX=/c\USERSTORE_SUFFIX=$AM_USER_STORE_SUFFIX" "$AM_CONFIG_FILE"
